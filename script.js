...
let activeView = "sales", activeCategory = "Tất cả", searchTerm = "", selectedTableId = "B05", managedTableId = "B05";
...
const elements = {
  workspace: $(".workspace"), tableGrid: $("#tableGrid"), categoryFilters: $("#categoryFilters"), menuGrid: $("#menuGrid"), menuCount: $("#menuCount"), menuSearch: $("#menuSearch"),
...
function renderOrders() {
  ...
  elements.orders.innerHTML = `${moduleHeader("Vận hành", "Quản lý đơn hàng", "Theo dõi và cập nhật tiến độ phục vụ", `<button class="btn btn-primary-action" data-view="sales"><i class="fa-solid fa-plus"></i> Tạo đơn mới</button>`)}
    ...
    <section class="module-card"><div class="module-toolbar"><div class="module-search"><i class="fa-solid fa-magnifying-glass"></i><input id="ordersSearch" type="search" value="${escapeHTML(orderSearch)}" placeholder="Tìm mã đơn, khách hàng, bàn..."></div><select id="ordersStatusFilter" class="compact-select"><option>Tất cả</option><option>Đang pha</option><option>Sẵn sàng</option><option>Chưa kết thúc</option><option>Hoàn tất</option><option>Đã hủy</option></select><span>${filtered.length} kết quả</span></div>${renderOrderTable(filtered)}</section>`;
...
function renderTableManagement() {
  ...
  elements.tables.innerHTML = `${moduleHeader("Không gian phục vụ", "Quản lý sơ đồ bàn", "Theo dõi trạng thái và điều phối khách theo thời gian thực", `<button class="btn btn-light-action" data-bs-toggle="modal" data-bs-target="#bookingModal"><i class="fa-regular fa-calendar-plus"></i> Đặt bàn</button><button class="btn btn-primary-action" data-table-action="start" data-table-id="${selected.id}"><i class="fa-solid fa-cash-register"></i> Mở order</button>`)}
    <div class="table-management-layout"><section class="module-card"><div class="module-toolbar"><div class="segmented-filter"><button class="${tableFilter === "all" ? "active" : ""}" data-table-filter="all">Tất cả (${tables.length})</button><button class="${tableFilter === "available" ? "active" : ""}" data-table-filter="available">Trống (${tables.filter((t) => t.status === "available").length})</button><button class="${tableFilter === "occupied" ? "active" : ""}" data-table-filter="occupied">Đang dùng (${tables.filter((t) => t.status === "occupied").length})</button><button class="${tableFilter === "reserved" ? "active" : ""}" data-table-filter="reserved">Đã đặt (${tables.filter((t) => t.status === "reserved").length})</button></div></div><div class="management-table-grid">${filtered.map((table) => `<button class="management-table-card ${table.status}${table.id === managedTableId ? " selected" : ""}" data-manage-table="${table.id}"><span class="table-visual"><i class="fa-solid fa-chair"></i></span><strong>Bàn ${table.id}</strong><small>${table.seats} chỗ</small><em>${tableStatusText(table.status)}</em><p>${table.detail}</p></button>`).join("")}</div></section>
    <aside class="module-card table-detail"><span class="large-table-icon ${selected.status}"><i class="fa-solid fa-chair"></i></span><p class="eyebrow">Bàn đang chọn</p><h2>Bàn ${selected.id}</h2><span class="status-pill ${selected.status}">${tableStatusText(selected.status)}</span><dl><div><dt>Sức chứa</dt><dd>${selected.seats} khách</dd></div><div><dt>Trạng thái</dt><dd>${selected.detail}</dd></div><div><dt>Khu vực</dt><dd>${selected.id.startsWith("A") ? "Cửa sổ" : selected.id.startsWith("B") ? "Trung tâm" : "Yên tĩnh"}</dd></div></dl><div class="detail-actions"><button class="btn btn-primary-action" data-table-action="start" data-table-id="${selected.id}">${tableSessions[selected.id] ? "Xem order của bàn" : "Mở order tại bàn"}</button><button class="btn btn-light-action" data-table-action="reserve" data-table-id="${selected.id}">Tạo lịch đặt</button>${selected.status === "reserved" && !tableSessions[selected.id] ? `<button class="btn danger-action" data-table-action="available" data-table-id="${selected.id}">Hủy giữ bàn</button>` : ""}</div></aside></div>`;
}
...
function renderSettings() {
  elements.settings.innerHTML = `${moduleHeader("Thiết lập hệ thống", "Cài đặt cửa hàng", "Các thay đổi chỉ được lưu trong phiên demo này", "")}
...
function renderTables() {
  elements.tableGrid.innerHTML = tables.map((table) => `<button class="table-card ${table.status}${table.id === selectedTableId ? " selected" : ""}" type="button" data-table-id="${table.id}" aria-pressed="${table.id === selectedTableId}" aria-label="Bàn ${table.id}, ${table.seats} chỗ, ${tableStatusText(table.status)}"><strong>Bàn ${table.id}</strong><small><i class="fa-solid fa-user-group"></i> ${table.seats} chỗ · ${tableStatusText(table.status)}</small><span>${table.detail}</span></button>`).join("");
...
function renderNotifications() { elements.notificationList.innerHTML = notifications.map((notice) => `<article class="notification-item${notice.read ? "" : " unread"}"><span><i class="fa-solid ${notice.icon}"></i></span><div><strong>${escapeHTML(notice.title)}</strong><p>${escapeHTML(notice.body)}</p><small>${notice.time}</small></div></article>`).join(""); const dot = $(".icon-button span"); if (dot) dot.hidden = !notifications.some((notice) => !notice.read); }
